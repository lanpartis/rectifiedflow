...
class PatchEmbd(nn.Module):
    def __init__(self, in_channels, image_size, dim, patch_size):
        ...
        self.conv_project = nn.Conv2d(in_channels=in_channels,out_channels=dim,kernel_size=patch_tuple,stride=patch_tuple, bias=False, padding_mode="zeros")
...
class DiTModel(nn.Module):
    def __init__(self, in_channels, image_size,hidden_size, patch_size, out_channels, num_heads, mlp_ratio, n_layers):
        super(DiTModel, self).__init__()
        self.hidden_size = hidden_size
        self.image_size = image_size
        self.patch_embd = PatchEmbd(in_channels=in_channels, image_size=image_size, dim=hidden_size, patch_size=patch_size)
        self.dits = [DiTBlock(hidden_size, num_heads, mlp_ratio) for _ in range(n_layers)]
    ...
    def _unpatchify(self, x, patch_size=(2, 2), height=32, width=32):
        ...
        # Reshape x to (bs, in_channels, num_patches_h, num_patches_w, H, W)
        x = x.view(bs,in_channels, num_patches_h, num_patches_w, H, W)

        # Permute x to (bs, in_channels, num_patches_h, H, num_patches_w, W)
        x = x.transpose(3, 4).contiguous()

        # Reshape x to (bs, height, width, in_channels)
        reconstructed = x.view(bs,in_channels, height, width, )

        return reconstructed
    
    def forward(self, x, c=None):
        x = self.patch_embd(x)
        pos_embed = pos_embd1d(torch.arange(self.patch_embd.num_patches), self.hidden_size)
        ...
        x = self._unpatchify(x,[self.patch_embd.patch_size]*2,self.image_size,self.image_size)
        return x
